...
function updateAttrs(oldVnode, vnode) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    assert.invariant(isUndefined$1(oldAttrs) || keys$1(oldAttrs).join(',') === keys$1(attrs).join(','), `vnode.data.attrs cannot change shape.`);
  }
...
function update(oldVnode, vnode) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    assert.invariant(isUndefined$1(oldProps) || keys$1(oldProps).join(',') === keys$1(props).join(','), 'vnode.data.props cannot change shape.');
  }
  ...
  return {
    ...
    set(newValue) {
      ...
      if (process.env.NODE_ENV !== 'production') {
        ...
        assert.invariant(!isObject$1(newValue) || isNull$1(newValue), `Invalid value "${newValue}" for "${propName}" of ${vm}. Value cannot be an object, must be a primitive value.`);
      }
...
function createPublicAccessorDescriptor(key, descriptor) {
  ...
  if (!isFunction$1(get)) {
    if (process.env.NODE_ENV !== 'production') {
      assert.invariant(isFunction$1(get), `Invalid compiler output for public accessor ${toString(key)} decorated with @api`);
    }
...
function validateMethodDecoratedWithWire(Ctor, methodName, descriptor) {
  if (isUndefined$1(descriptor) || !isFunction$1(descriptor.value) || isFalse$1(descriptor.writable)) {
...
function fallbackElmHook(elm, vnode) {
  ...
  if (isTrue$1(owner.renderer.syntheticShadow)) {
    const {
      data: {
        context
      }
...
function allocateChildrenHook(vnode, vm) {
  ...
  if (isTrue$1(vm.renderer.syntheticShadow)) {
    // slow path
...
function i(iterable, factory) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    assert.isFalse(isUndefined$1(iterable[SymbolIterator]), `Invalid template iteration for value \`${toString(iterable)}\` in ${vmBeingRendered}. It must be an array-like object and not \`null\` nor \`undefined\`.`);
  }
...
function dc(sel, Ctor, data, children) {
  if (process.env.NODE_ENV !== 'production') {
    ...
    assert.isTrue(arguments.length === 3 || isArray(children), `dc() 4nd argument data must be an array.`);
...
function evaluateTemplate(vm, html) {
  if (process.env.NODE_ENV !== 'production') {
    assert.isTrue(isFunction$1(html), `evaluateTemplate() second argument must be an imported template instead of ${toString(html)}`); // in dev-mode, we support hot swapping of templates, which means that
    // the component instance might be attempting to use an old version of
...
function createVM(elm, def, options) {
  ...
  if (isFalse$1(renderer.ssr) && hasWireAdapters(vm)) {
...
function recursivelyDisconnectChildren(vnodes) {
  for (let i = 0, len = vnodes.length; i < len; i += 1) {
    ...
    if (!isNull$1(vnode) && isArray(vnode.children) && !isUndefined$1(vnode.elm)) {
...
function scheduleRehydration(vm) {
  if (isTrue$1(vm.renderer.ssr) || isTrue$1(vm.isScheduled)) {
    return;
  }
  ...
  setProperty(node, key, value) {
    ...
    if (node.type === HostNodeType.Element) {
      ...
      if (node.name === 'input' && attrName === 'value') {
        return isNull(value) || isUndefined(value) ? this.removeAttribute(node, 'value') : this.setAttribute(node, 'value', value);
      }
    }
...
function serializeAttributes(attributes) {
  return attributes.map(attr => attr.value.length ? `${attr.name}=${JSON.stringify(htmlEscape(attr.value))}` : attr.name).join(' ');
}
...
function renderComponent$1(tagName, Ctor, props = {}) {
  if (!isString(tagName)) {
...
export { BaseLightningElement as LightningElement, api, createContextProvider, getComponentDef, isComponentConstructor, readonly, register, registerComponent, registerDecorators, registerTemplate, renderComponent$1 as renderComponent, sanitizeAttribute, setFeatureFlag, setFeatureFlagForTest, track, unwrap$1 as unwrap, wire };
