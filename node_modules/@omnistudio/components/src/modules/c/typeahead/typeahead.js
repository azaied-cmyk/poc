...
import { debounce, delay } from "c/asyncUtils";
import { typeaheadLabels as translatedLabels } from "c/salesforceUtils";
...
export default class VlocityTypeahead extends LightningElement {
  ...
  get typeaheadFormElementClass() {
    return `${this.theme}-combobox__form-element ${
      this.iconPosition !== null
        ? `${this.theme}-input-has-icon ${this.theme}-input-has-icon_${
            this.iconPosition
          }${
            this.theme === "nds"
  ...
  filterOptions() {
    ...
    this.internaldata = this.options.filter(item => {
      ...
      if (this._value) {
        return (
          item.name.toLowerCase().includes(this._value.toLowerCase()) ||
          (item.displayName &&
            item.displayName
              .toLowerCase()
              .includes(this._value.toLowerCase())) ||
          (item.description &&
            item.description.toLowerCase().includes(this._value.toLowerCase()))
        );
  ...
  get getDropdownClasses() {
    let dropdownClass = `${this.theme}-dropdown ${this.theme}-dropdown_length-with-icon-7 ${this.theme}-dropdown_fluid`;
    if (this.theme === "nds") {
