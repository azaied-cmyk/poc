...
import currencyjs from "c/currencyjs";
import { debounce, formatDate, isDate } from "c/utility";
import { find, findIndex, cloneDeep } from "c/lodash";
import { dataTableLabels as translatedLabels } from "c/salesforceUtils";
...
export default class DataTable extends LightningElement {
  ...
  initRecord(value) {
    ...
    this._records = value.map((item, index) => {
        ...
        if (
          ...
          if (
            ...
            if (cell.type === "date" && cell.dateFormat && cell.data.value) {
              cell.data.value = formatDate(cell.data.value, dateFormat);
            }
          }
      ...
      if (this.filteredData) {
        ...
        this.filteredData.map(item => {
          ...
          let groupItem = {
            ...
            editMode: false,
            displayClass: `${this.theme}-data-table-row-cell tableRowCell`,
            originalItemIndex: item.originalIndex,
            selectRow: this.selectAllRow || item.selectrow,
            isDeleteActive: this.rowDeleteDependentColumn
              ? item[this.rowDeleteDependentColumn] && this.rowDelete
    ...
    if (!groupSummaryField.methodToCalculateSummary) {
      let doSum = function(groupItemsToAdd) {
        ...
        groupItemsToAdd.forEach(item => {
          sum = currencyjs(item[groupSummaryField.name])
            .add(sum)
            .format(true);
        });
        return sum;
  ...
  sortRows = row => {
    let col = row.find(_item => _item.fieldName === this.sortingObj.sortField);
    if (col) {
    ...
    let formatValue = a => {
      ...
      let regex =
        type === "number" || type === "currency"
          ...
          : /[^a-zA-Z0-9.]/g;
      a =
        typeof keyValue == "string" && type !== "date"
          ? keyValue.replace(regex, "")
          : keyValue;
      a =
        typeof a == "string" && type !== "date"
          ? !/[a-zA-Z]/.test(a)
            ? Number(a)
            : a.toLowerCase()
          : a;
  ...
  saveDataRow(event) {
    ...
    const btnElem = element.querySelector("c-button");
  ...
  toggleGroupedRow(event) {
    ...
    this.tablePageGroupData[index].isOpen = !this.tablePageGroupData[index]
      .isOpen;
    if (
      !this.activeGroups &&
      (this.rowDelete || this.rowLevelEdit || this.cellLevelEdit)
    ) {
      this._activeGroupMap[
        this.tablePageGroupData[index].groupName
  ...
  deleteRow(event) {
    event.stopPropagation();
    const iconElem = event.target.parentElement.querySelectorAll("c-icon");
    const buttonElem = event.target.parentElement.querySelector(
      ".rowLevelCheck"
    );
    ...
    this.fireEvent("delete", this.records[this.deleteRowData]);
    this.fireEvent("update");
    let recordsCopy = [...this.records];
    recordsCopy.splice(this.deleteRowData, 1);
  ...
  onDragLeave(e) {
    ...
    if (
      ...
      e.currentTarget.classList.remove(`${this.theme}-border-top-20-lightgray`);
    }
  ...
  selectGroupedRow(isSelected, rowIndex) {
    ...
    if (itemIndex !== -1)
      this.tableGroupData[groupIndex].groupItems[
        itemIndex
