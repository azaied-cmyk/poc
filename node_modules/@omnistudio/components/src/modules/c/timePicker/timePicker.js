...
import { unitName, determineMaxGranularity } from "c/vlocDayJSHelpers";
import dayjs from "c/dayjs";
import { startOfDay, endOfDay } from "c/date_fns";
import { timePickerLabels as translatedLabels } from "c/salesforceUtils";
...
export default class VlocityTimePicker extends LightningElement {
  ...
  get startTime() {
    if (!this._startTime) {
      this._startTime = this._min || startOfDay(new Date());
    }
    return this._startTime;
  }
  get endTime() {
    if (!this._endTime) {
      this._endTime = this._max || endOfDay(new Date());
    }
  ...
  setValue(value) {
    if (value) {
      ...
      if (this._displayValue) {
        ...
        let validator = (this.firstRender
          ? this.checkValidity
          : this.showHelpMessageIfInvalid
        ).bind(this);
        if (this._connected && !validator()) {
          this._displayValue = oldVal;
          this.setSelectedOption();
          validator();
        }
      }
  ...
  renderedCallback() {
    ...
    if (input && this.firstRender) {
      ["blur", "focus", "change"].forEach(event => {
        ...
        this.registerListener(input, event, eventCallback);
      }, this);
  ...
  clearFocus() {
    let elements = this.template.querySelectorAll(`.${this.theme}-has-focus`);
