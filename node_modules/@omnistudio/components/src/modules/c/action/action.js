...
import { isEqual, get } from "c/lodash";
import sldsTemplate from "./action_slds.html";
import ndsTemplate from "./action_nds.html";
import { getDefaultTrackingData, initInteraction } from "c/actionUtility";
import vtag from "c/oaVtag";
import { isSalesforcePlatform } from "c/sdkUtility";
...
export default class Action extends BaseFlexElementMixin(LightningElement) {
  ...
  validateInterpolatedAction(setValue) {
    if (setValue) {
      ...
      this.oldInterpolatedFields.action = { ...this.interpolatedAction };
    }
  }
  ...
  resolveActionDetails() {
    ...
    return new Promise(resolve => {
      ...
      this.configAction(action).then(data => {
        resolve(data);
      });
    });
  }
  ...
  configAction(action) {
    return new Promise((resolve, reject) => {
      ...
      switch (type) {
        case "Vlocity Action":
          setVlocityAction(action, this.disableCache).then(result => {
            //If vlocity action has custom label then use that coming from action properties, if not, use action display Label
            result.displayName = result[namespace + "CustomLabelName__c"]
              ? this.stateAction &&
                get(
                  this.card,
                  this.stateAction.displayName.replace(/[{}]/g, "")
                )
              : result.displayName;
            let url = result[this.nsPrefix + "Url__c"];
            this.actionUrl = url ? url : "";
            this.saveActionData(result);
            this.actionDetails.type = "Vlocity Action";
            resolve(result);
          });
          break;
        case "OmniScript":
          setOsAction(action).then(response => {
            this.saveActionData(response);
            resolve(response);
          });
          break;
        case "Event":
          setEventAction(action).then(response => {
            this.saveActionData(response);
            resolve(response);
          });
          break;
        ...
        case "PubSub":
          ...
          break;
        case "Flyout":
          data = this.setFlyoutAction(action);
          this.saveActionData(data);
          resolve(data);
          break;
        case "Custom":
          setCustomAction(action).then(response => {
            if (response) {
  ...
  fireSelectCard(event) {
    let detail = {
      value:
        event.currentTarget && event.currentTarget.name === "selectableAction"
  ...
  setFlyoutAction(action) {
    this.isFlyout = action.parent !== "menu" || action.isMenuItem;
    let clonedAction = {
      ...action,
      iconName:
   ...
   */
  @api async executeAction(event, card) {
    this.resetVariables();
    ...
    this.stateAction = interpolateElement(this.stateAction, currentTarget);
    ...
    if (
      ...
      this.fireSelectCard(event);
      return Promise.resolve("Action Performed");
    }
      ...
      switch (type) {
        case "Vlocity Action":
          if (!this.isPageRefSet) {
            fireVlocityAction().then(response => {
              this.fireNavAction(response).then(data => {
                resolve(data);
              });
            });
          } else {
            this.fireNavAction().then(data => {
              resolve(data);
            });
          }
          break;
        case "Custom":
          if (!this.isPageRefSet) {
            fireCustomAction().then(response => {
              this.fireNavAction(response).then(data => {
                resolve(data);
              });
            });
        ...
        case "PubSub":
          ...
          break;
        case "cardAction":
          this.fireCardAction(event).then(response => {
            this.setOaObj();
            resolve(response);
          });
          break;
        ...
        case "Flyout":
          ...
          resolve("Flyout Toggled");
          break;
        case "updateOmniScript":
          this.fireUpdateOSaction().then(detail => {
            resolve(detail);
          });
          break;
        case "OmniScript":
          performOsAction().then(response => {
            this.fireOsAction(response).then(data => {
              resolve(data);
            });
          });
          break;
  ...
  onCloseModal() {
    ...
    if (this.actionDetails.isMenuItem) {
      this.dispatchEvent(
  ...
  get actionItemClass() {
    ...
    classes += this.actionDetails?.isMenuItem
