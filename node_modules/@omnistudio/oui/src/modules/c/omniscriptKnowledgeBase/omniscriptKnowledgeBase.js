...
import tmpl from './omniscriptKnowledgeBase.html';
...
export default class OmniscriptKnowledgeBase extends LightningElement {
    ...
    connectedCallback() {
        ...
        if (this.getAttribute('data-omni-key') == null && this.layout === 'newport') {
        ...
        renderPromise.then(() => {
            ...
            this._isConfiguredOnOmniScript = this.getAttribute('data-omni-key') == null ? false : true;
            this._omniNewportClass = this._isConfiguredOnOmniScript ? '' : 'via-nds';
            this._isStepChartTop = this.getAttribute('data-stepchart-placement') === 'top' ? true : false;
            ...
            pubsub.register(`${this.omniKey}omnikboptionschange`, this._knowledgeOptionsChangeObj);

            pubsub.register(`${this.omniKey}omnikbarticlebodychange`, this._kbArticleOptionsObj);
            if (this.knowledgeOptions != null) {
                ...
                if (this._knowledgeBaseOptions.allCustomLabels) {
                    this._customLabelsUtil = Object.assign({}, this._customLabelsUtil, this._knowledgeBaseOptions.allCustomLabels);
                }
                this.handleKnowledgeOptions(this._knowledgeBaseOptions);
            } else {
                pubsub.fire(`${this.omniKey}omnikbinitilized`, 'data', 'knowledge component initilized');
            }
        });
    ...
    handleKnowledgeOptions(data) {
        ...
        if (this._knowledgeBaseOptions) {
            ...
            this.canShowKbOnStep = this._isConfiguredOnOmniScript ? this._knowledgeBaseOptions.isKbEnabledOnStep : this.canShowKbOnStep;
            if (this.canShowKbOnStep && this.inputSearchKeyword) this.kbInvokeAction(this._knowledgeBaseOptions);
    ...
    searchKnowledgeArticle(event) {
        if ((event && event.keyCode !== 13) || event.target.value === this.inputSearchKeyword) return;

        this.inputSearchKeyword = event.target.value;
        let kbOptions = this._knowledgeBaseOptions ? JSON.parse(JSON.stringify(this._knowledgeBaseOptions)) : null;
        if (kbOptions) {
    ...
    kbInvokeAction(params) {
        ...
        return this._actionUtilClass.executeAction(params, null, this).then(resp => {
            if (!resp.error && resp.result) {
                const articlesResults = resp.result.vlcKnowledge ? resp.result.vlcKnowledge.knowledgeItems : [];
                this.articlesResults = articlesResults.map(article => {
                    const title = article.title || '';
                    article.openLabel = this._customLabelsUtil.OmniScriptOpenFormatted.replace('{0}', title);
                    article.modalLabel = this._customLabelsUtil.OmniScriptModalFormatted.replace('{0}', title);
                    return article;
                });
                this._numArticlesLabel = this._customLabelsUtil.OmniKnowledgeArticlesCount.replace('{0}', articlesResults.length);
            }
        });
    }
    ...
    get classToggleWidth() {
        ...
        containerClasses = this.layout === 'newport' ? containerClasses + ' nds-knowledge-wrapper' : containerClasses;
     ...
     */
    focusExpandButton() {
        const button = this.template.querySelector(`button.${this._themeKB}-knowledge-expand`);
        if (button) {
            button.focus();
        }
    }
    ...
    openModal(evt) {
        ...
        this._article = this.articlesResults[evt.currentTarget.getAttribute('data-info')];
        this.getArticleBody();
    }
    ...
    getArticleBody() {
        ...
        if ((this._knowledgeBaseOptions.lightningKBEnable != null && this._knowledgeBaseOptions.lightningKBEnable) === true) {
    ...
    kbArticleInvokeAction(params) {
        ...
        return this._actionUtilClass.executeAction(params, null, this).then(resp => {
            if (!resp.error && resp.result) {
                let body;
                let articleDetails = resp.result.vlcArticleBody ? resp.result.vlcArticleBody : [];
                    ...
                    for (let count = 0; count < articleDetails.length; count++) {
                        for (let key in articleDetails[count]) {
                            if (articleDetails[count].hasOwnProperty(key)) {
                                if (count !== 0) body += this.layout === 'lightning' ? '<br/><hr/>' : '<br/>';
                ...
                this.articleBodyResults.articleLink = '/articles/' + this._article.aType + '/' + this._article.urlName;
                this.articleBodyResults.setFocusToArticle = true;

                if (this._isOpenOmniScript) {
                    let copyResult = JSON.parse(JSON.stringify(this.articleBodyResults));
                    pubsub.fire(`${this.omniKey}omnikbarticlebodyresult`, 'data', copyResult);
                } else {
                    this._kbModal = true;
    ...
    setFirstFocusableElement() {
        this.firstFocusableElement = this.template.querySelector(`.${this._themeKB}-modal__container > header`);
    ...
    setLastFocusableElement() {
        this.lastFocusableElement = this.template.querySelector('[role="dialog"] [data-button]');
    ...
    sendDataToDebugConsole(params, resp, label, element) {
        ...
        const debugData = {
            params: sendParams,
            response: sendResp,
            element: { label: label, elementType: element && element.type ? element.type : element },
        };
    ...
    disconnectedCallback() {
        pubsub.unregister(`${this.omniKey}omnikboptionschange`, this._knowledgeOptionsChangeObj);
        pubsub.unregister(`${this.omniKey}omnikbarticlebodychange`, this._kbArticleOptionsObj);
    }
}
