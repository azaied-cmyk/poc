...
import { VlocityTrack } from 'c/omniscriptRestApi';
 ...
 * @returns {void}
 */
export function handleTimeTracking(headerComp, stepEleOrActionComp, eventNameOrEleJsonDef, additionalData, bStep = true) {
    if (bStep === true) {
        handleStepTimeTracking(headerComp, stepEleOrActionComp, eventNameOrEleJsonDef, additionalData);
    } else {
        handleActionTimeTracking(headerComp, stepEleOrActionComp, eventNameOrEleJsonDef, additionalData);
    }
}
 ...
 * @returns {void}
 */
function handleStepTimeTracking(comp, stepElement, trackingEventName = 'StepActionTime', additionalData = {}) {
    ...
    if (comp.scriptHeaderDef.propSetMap.timeTracking && comp.jsonDef.sOmniScriptId && comp.scriptHeaderDef.omniAnalyticsEnabled !== true) {
        const dataToSend = getCommonTrackingData(comp, stepElement, comp.scriptHeaderDef, comp._elementValueObj);
        ...
        if (stepElement.st && additionalData.elapsedTime) {
            ...
            invokeApexTrack(dataToSend);
            updateTrackingDataJson(comp._elementValueObj, stepElement, additionalData.elapsedTime, comp);
 ...
 * @returns {void}
 */
function handleActionTimeTracking(headerComp, actionComp, element, additionalData = {}) {
    // Processes time tracking logic when time tracking is enabled and processed from the Header component. This should
    // not fire when OA is enabled
    if (
        ...
        const rootIndex = targetElement.rootIndex >= 0 ? targetElement.rootIndex : targetElement.indexInParent;
        const elementJsonDef = headerComp.jsonDef.children[rootIndex];
        const dataToSend = {
            ...getCommonTrackingData(actionComp, element, headerComp.scriptHeaderDef, headerComp._elementValueObj, additionalData),
            TrackingEvent: 'StepActionTime',
        };

        invokeApexTrack(dataToSend);

        updateTrackingDataJson(headerComp._elementValueObj, elementJsonDef, dataToSend.ElapsedTime, headerComp);
 ...
 * @returns {void}
 */
export function updateTrackingDataJson(dataJson, elementJsonDef, elapsedTime, comp) {
