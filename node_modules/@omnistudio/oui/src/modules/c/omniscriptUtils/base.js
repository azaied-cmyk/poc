...
import { handleMergeField, isRepeatNotation, mergeJSONLogic } from 'c/omniscriptInternalUtils';
import { mergeWith } from 'c/lodash';
import { getCommonTrackingData } from 'c/omniscriptTrackingServiceUtils';
import vtag from 'c/oaVtag';

const VALIDREPEATTYPE = { 'Edit Block': '' };
...
export function applyCallRespBase(data, comp, bApi, bValidation) {
    ...
    let remainder = {};
    let cacheJson = {};
    const uiElements = (comp.scriptHeaderDef && comp.scriptHeaderDef.labelMap) || {};
    const acUiElements = (comp.scriptHeaderDef && comp.scriptHeaderDef.acUiElements) || {};
    const apiUiElements = (comp.scriptHeaderDef && comp.scriptHeaderDef.apiUiElements) || {};
        ...
        if (uiElements.hasOwnProperty(item)) {
            ...
            if (acUiElements.hasOwnProperty(item)) {
                ...
                if (childCompList && childCompList.length > 0) {
                    ...
                    if (childCompList[0].jsonDef.propSetMap.repeat || VALIDREPEATTYPE.hasOwnProperty(childCompList[0].jsonDef.type)) {
                        // we are not going to create new component for now
                        let compSize = childCompList.length;
                        const compListEmpty = typeof childCompList[0].isEmpty === 'function' && childCompList[0].isEmpty();
                        ...
                        if (childCompList[0].jsonDef.propSetMap.repeatLimit != null) {
                            dataSize = Math.min(dataSize, childCompList[0].jsonDef.propSetMap.repeatLimit + 1);
                        }
                        ...
                        if (!bDataArray) {
                            ...
                            for (let i = 0; i < maxEditSize; i++) {
                                if (i === 0 && compListEmpty) {
                                    ...
                                    childCompList[i].applyCallResp(data[item][i], bApi, bValidation);
                                }
                            }
                ...
                if (apiUiElements.hasOwnProperty(item)) {
                    if (bValidation) {
                        cacheJson.vlcValidationErrors = Object.assign({}, cacheJson.vlcValidationErrors, {
                            [item]: data[item],
                        });
                    }
...
export function queryOmniComp(comp, omniKey, bMulti) {
    ...
    if (!bMulti) {
        ...
        returnComp = comp.template.querySelector('[data-omni-key="' + omniKey + '"]');
        ...
        if (compList.length === 0) {
            compList = comp.template.querySelectorAll('[data-omni-key="' + omniKey + '"]');
 ...
 * @returns {Void}
 */
export function sendOmniPubsubEvent(comp, eventName = 'omniscript_event', additionalData, element) {
    if (comp && comp.jsonDef && comp.scriptHeaderDef) {
        ...
        if (msg && Object.keys(msg).length > 0) {
            ...
            msg = JSON.parse(
                handleMergeField(msgText, comp._jsonData, comp.scriptHeaderDef.labelMap, isRepeatNotation(msgText) ? comp.jsonDef.JSONPath : null),
            );
        } else {
...
export function handleMessaging(comp, pubsubEventName, additionalData, element, oaEventName) {
    const properties = getDefaultMessagingProperties(comp, element);
    ...
    if (
        ...
        (properties.wpm || properties.ssm || properties.pubsub || properties.oaEnabled)
    ) {
        ...
        const messagePayload =
            pubsubEventName !== 'omniscript_datajson'
                ? getCommonTrackingData(comp, element, comp.scriptHeaderDef, comp._jsonData, additionalData)
                : {};
 ...
 * @returns {Void}
 */
export function setSessionStorageMessage(messageData, messageName = 'OmniScript-Messaging') {
    if (messageData) {
        let messages = [];
