...
import tmpl from './omniscriptBaseAction.html';
import tmpl_nds from './omniscriptBaseAction_nds.html';
import { debounce } from 'c/asyncUtils';
import { showToast } from 'c/omniscriptInternalUtils';
import { OmniscriptBaseActionUtil } from 'c/omniscriptActionUtils';
...
export default class OmniscriptBaseAction extends OmniscriptGroupElement {
    ...
    handleResponse(resp, element) {
        ...
        if (resp.error) {
            ...
            // eslint-disable-next-line consistent-return
            return this.dispatchOmniEventUtil(this, { apiResponse: resp.result }, 'omniactionbtn');
        }
    ...
    handleResponseError(resp, element) {
        ...
        if (resp.result && !resp.result.vlcValidationErrors) {
            this.sendErrorModalUtil(resp.errorMsg, this._isBtn, element, this.scriptHeaderDef, this);
        }
    ...
    connectedCallback() {
        ...
        if (this.jsonDef && this.jsonDef.level > 0) {
            this._isBtn = true;
            this.extraclass = this._theme + '-button_stretch' + (this._propSetMap.label ? '' : ' ' + this._theme + '-box');
        } else {
    ...
    initCompVariables() {
        ...
        this._timeTrackingEnabled =
            ...
            this.evaluateMessagingUtil(this.jsonDef.propSetMap, this.scriptHeaderDef.omniAnalyticsEnabled);
    }
    ...
    handleToastCompletion(title, message) {
        if (this._propSetMap.toastComplete && this._propSetMap.invokeMode !== 'fireAndForget') {
    ...
    runAction(element) {
        ...
        return this.sendErrorModalUtil(this.allCustomLabelsUtil.OmniStepValidationError, this._isBtn, element, this.scriptHeaderDef, this);
    }
    ...
    processAction(element) {
        return this._actionUtilClass
            ...
            .catch(error => {
                ...
                this.evaluateSpinner(false, element);
                this.sendErrorModalUtil(error.errorMsg, this._isBtn, element, this.scriptHeaderDef, this);
    ...
    checkActionValidity(element) {
        return (
            ...
            (!this.scriptHeaderDef.hasInvalidElements && element.propSetMap.validationRequired === 'Step') ||
            this.skipValidation()
        );
    }
