...
import { handleMergeField, isRepeatNotation, autoAdvance } from 'c/omniscriptInternalUtils';
...
export const OmniscriptBaseMixin = Base =>
    class extends Base {
        ...
        omniUpdateDataJson(input, aggregateOverride = false) {
            if (this.omniJsonDef && this.omniJsonDef.index !== undefined && this.omniJsonDef.name) {
                const detail = {
                    data: input,
        ...
        omniApplyCallResp(response, usePubsub = false) {
            ...
            if (usePubsub) {
                pubsub.fire(this.omniScriptHeaderDef.uuid + 'omniactionbtn', 'data', { detail: detail });
            } else {
        ...
        omniSaveState(input, key, usePubsub = false) {
            ...
            if (saveStateKey) {
                ...
                if (!usePubsub) {
                    ...
                    pubsub.fire(this.omniScriptHeaderDef.uuid + 'omnicustomsavestate', 'data', state);
                }
            }
        }
        ...
        omniGetSaveState(key) {
            ...
            if (saveStateKey) {
                if (this.omniCustomState && this.omniCustomState[saveStateKey]) {
                    ...
                    state = JSON.parse(JSON.stringify(this.omniCustomState[saveStateKey]));
                }
            }
            return state;
        }
        ...
        omniGetMergeField(mergeFieldString) {
            if (mergeFieldString && this.omniJsonData && this.omniScriptHeaderDef && this.omniJsonDef) {
