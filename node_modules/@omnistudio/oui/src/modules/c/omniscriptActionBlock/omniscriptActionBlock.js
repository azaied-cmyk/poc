...
import { initializeAction } from 'c/omniscriptActionUtils';
...
export default class OmniscriptActionBlock extends OmniscriptBaseAction {
    ...
    processAction(element) {
        ...
        if (element && element.propSetMap && element.propSetMap.actions && element.propSetMap.actions.length > 0) {
            // stores an array of executed action promises
            const actionPromises = element.propSetMap.actions.map(action => {
                const omniShowAction = this.evalConditionUtil(action, 'show', this, true);
                ...
                if (omniShowAction) {
                    ...
                    return actionInstance.executeAction(null, null, this, null, null).catch(error => {
        ...
        return this.handleError(element, { errorMsg: this.allCustomLabelsUtil.OmniNoActionsError });
    }
    ...
    handleResponseError(resp, element) {
        ...
        this.sendErrorModalUtil(
            resp.errorMsg,
            this._isBtn === true || (this._isBtn === false && element.propSetMap.applyIfError),
            element,
        ...
        if (element.propSetMap.applyIfError) {
            if (this._isBtn) {
                this.dispatchOmniEventUtil(this, { apiResponse: resp.result }, 'omniactionbtn');
            } else {
    ...
    getBoundsForDesigner() {
        const container = this.template.querySelector('.omni-designer-action-container');
    ...
    sendDataToDebugConsole(params, resp, label, element) {
        if (element && element.type === 'Rest Action') {
            sendHttpDataToDebugConsole(this, params, resp, label, element, 'omniactiondebug');
        } else {
