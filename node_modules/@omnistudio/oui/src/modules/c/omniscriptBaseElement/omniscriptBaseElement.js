...
import { applyCallRespBase, queryOmniComp, dispatchOmniEvent } from 'c/omniscriptUtils';
import {
    handleMergeField,
    evalCondition,
    mergeJSONLogic,
...
function scheduleAsync(callback) {
    ...
    return SUPPORTS_IDLE_CALLBACK ? requestIdleCallback(callback) : requestAnimationFrame(callback);
}
...
export default class OmniscriptBaseElement extends LightningElement {
        ...
        if (this.scriptHeaderDef && this.jsonDef && this._propSetMap) {
            if (this._propSetMap.show) {
                ...
                if (conditionType === 'Hide if False') {
                    this.omniShow();
                } else if (['Readonly if False', 'Optional if False'].includes(conditionType)) {
    ...
    omniShow() {
        ...
        if (this._isDesignMode) {
            ...
            if (this._jsonData && this.jsonDef && this._propSetMap && this._propSetMap.show && this.designerHideElementFlag) {
        ...
        if (this._jsonData && this.jsonDef && this._propSetMap && this._propSetMap.show) {
            ...
            if (this.jsonDef.bShow === undefined || newShow !== oldShow) {
                ...
                this.dispatchOmniEventUtil(
                    this,
                    { path: this._jsonPath, elementId: this._elementId, value: this._omniShow ? true : false, node: 'bShow' },
                    'omnisetshow',
                );
                ...
                if (
                    (this.scriptHeaderDef.apiUiElements && this.scriptHeaderDef.apiUiElements.hasOwnProperty(this._elementId)) ||
                    (this.scriptHeaderDef.aggElements && this.scriptHeaderDef.aggElements.hasOwnProperty(this._elementId))
                ) {
                    this._aggregateNodesPath = [];
                    this.dispatchOmniEventUtil(this, this.createAggregateNode(true), 'omniaggregate');
                }
    ...
    applyCtrlWidth() {
        ...
        if (this.jsonDef && this._propSetMap) {
            if (this._propSetMap.controlWidth != null && this.jsonDef.level !== 0) {
                ...
                this.classList.add(this._theme + '-medium-size_' + this._propSetMap.controlWidth + '-of-12');
            }
    ...
    renderedCallback() {
        ...
        if (this._initialRender) {
            ...
            if (this.jsonDef && !this.jsonDef.bInit && sdata == null && this.defaultValue != null) {
            ...
            if (this.jsonDef.bInit !== true) {
                this.dispatchOmniEventUtil(
                    this,
                    { path: this._jsonPath, elementId: this._elementId, value: true, node: 'bInit' },
                    'omniupdatejsondef',
                );
            }
            ...
            if (this._isDesignMode) {
                this._throttledDesignRender = debounce(this.dispatchDesignerRenderElementEvent.bind(this), 250);
                // eslint-disable-next-line @lwc/lwc/no-async-operation
    ...
    triggerParentChain(event) {
        ...
        Promise.resolve().then(() => {
            ...
            if (didFire) {
                this.promiseQueueForChildren = this.promiseQueueForChildren.filter(detail => {
    ...
    getBoundsForDesigner() {
        ...
        return Object.assign({}, getBoundingClientRectClone(), {
            width: this._theme === 'slds' && this.classList.contains('slds-p-right_small') ? boundingClientRect.width - 12 : boundingClientRect.width,
            paddingTop: 0,
            paddingLeft: 0,
            paddingBottom: 0,
            paddingRight: 0,
    ...
    dispatchDesignerRenderElementEvent() {
        ...
        if (!/display/.test(cachedStyle)) {
            this.setAttribute('style', `${cachedStyle ? cachedStyle + ';' : ''} display: block;`);
        }
        const currentBounds = this.getBoundsForDesigner();
        if (this.previousBounds && JSON.stringify(this.previousBounds) === JSON.stringify(currentBounds)) {
            // eslint-disable-next-line @lwc/lwc/no-async-operation
            scheduleAsync(this._throttledDesignRender);
            return didFire;
        }
    ...
    suspendEvents(state) {
        if (state.omniscriptId !== (this.jsonDef.sOmniScriptId || this.jsonData.recordId)) return;
    ...
    connectedCallback() {
        if (this._isDesignMode) {
            let elmNameEvent;
            if (this.jsonDef.type !== 'Edit Block') elmNameEvent = this.jsonDef.name + 'widthchange';
        ...
        if (this.jsonDef) {
            ...
            if (this.jsonDef && this.jsonDef.response != null) {
                ...
                if ((this.jsonDef.children && this.jsonDef.children.length > 0) || (this.jsonDef.childrenC && this.jsonDef.childrenC.length > 0)) {
            ...
            if (this.jsonDef.newClone) {
                ...
                this.dispatchOmniEventUtil(this, this.createAggregateNode(false, 'add'), 'omniaggregate');
            }
        }

        if (this._isDesignMode) {
            this.template.addEventListener('omnidesignerrenderelement', this.triggerParentChain.bind(this));
    ...
    get canRepeat() {
        ...
        return this._propSetMap.repeat && (this._propSetMap.repeatLimit == null || count <= this._propSetMap.repeatLimit);
    }
    ...
    handleRepeat(operation, seed) {
        ...
        if (operation === 'delete' || operation === 'eblastdel') {
            ...
            this.dispatchOmniEventUtil(this, this.createAggregateNode(false, 'delete'), 'omniaggregate');
    ...
    propToMultiLang(properties) {
        if (!this._isDesignMode) {
            ...
            handleMultiLangLabel(this.jsonDef.type, this._propSetMap, this.allCustomLabelsUtil);
        }
    }
    ...
    handleActionElementWidth(state) {
        ...
        this.classList.remove(this._theme + '-medium-size_' + this._lastWidth + '-of-12');
    ...
    get designerHideElementFlag() {
        ...
        return getDesignerHideElementFlag(this.jsonDef.sOmniScriptId || this.jsonData.recordId);
    }
}
