...
import { getElementValue, isArrayEqual } from 'c/omniscriptInternalUtils';
...
export const OmniscriptOptionsMixin = Base => {
    return class extends Base {
        ...
        initCompVariables() {
            ...
            if (this.scriptHeaderDef.rtpSeed) {
                ...
                if (this._propSetMap.controllingField.type === 'SObject') {
                    key = this._propSetMap.optionSource.source + '/' + this._propSetMap.controllingField.source;
                } else if (this._propSetMap.controllingField.type === 'Custom') {
                    key = this._propSetMap.controllingField.element + '/' + this._propSetMap.controllingField.source;
                } else if (this._propSetMap.optionSource.type === 'SObject' || this._propSetMap.optionSource.type === 'Custom') {
                    key = this._propSetMap.optionSource.source;
                }

                if (this._propSetMap.controllingField.type === 'SObject' || this._propSetMap.controllingField.type === 'Custom') {
            ...
            this._tempOptions = this.prependOptions(this._initialOptions, this._seedOptions);
        ...
        setElementFormattedValue() {
            if (this._realtimeOptions.length > 0 && this.elementValue != null) {
                let tempElementFormattedValue = '';
                let numberOfElementValues = 0;
                const elementValueArr = this.elementValue.split(';');
        ...
        getOptions(initialOptions) {
            ...
            const elementValue = getElementValue(elementName, this.jsonData, this.scriptHeaderDef.labelMap, this.jsonDef.JSONPath || null);
            ...
            if (newOptions && !isArrayEqual(newOptions, this._tempOptions)) {
                ...
                const checkedResult = this.checkOptions(this._realtimeOptions, this.jsonDef.response);

                // clears jsonData if no valid responses are found
                if (!checkedResult.valid || (checkedResult.valid && checkedResult.dataToApply === '')) {
                    this.applyCallResp(null);
                }
                // applies jsonData if valid options exist
                    ...
                    if (checkedResult.dataToApply !== this.jsonDef.response) {
                        this.applyCallResp(checkedResult.dataToApply);
