...
import { OmniscriptDocusignSignatureActionUtil, OmniscriptRemoteActionUtil } from 'c/omniscriptActionUtils';
import { getNamespaceDotNotation, getUrl } from 'c/omniscriptInternalUtils';
import tmpl from './omniscriptDocusignSignatureAction.html';
...
export default class OmniscriptDocusignSignatureAction extends OmniscriptBaseAction {
    ...
    connectedCallback() {
        ...
        window.addEventListener('message', event => {
            try {
                let docusignStatus =
                    typeof event.data === 'string' && event.data.indexOf('docusignPostMessage') !== -1 ? JSON.parse(event.data) : event.data;
                if (typeof docusignStatus === 'object' && docusignStatus.docusignPostMessage) {
                    ...
                    if (docusignStatus.docuSignViewPdfPageloaded) {
                        this._iframeComp.contentWindow.postMessage({ pdfData: this._pdfData, docusignPostMessageViewPdf: true }, '*');
                    } else {
                        ...
                        if (docusignStatus) {
                            ...
                            } else if (docusignStatus === 'decline') docusignStatus = 'Declined';
                            ...
                            if (this._envelopeIdArray[0]) {
                                ...
                                this.dispatchOmniEventUtil(this, { apiResponse: { [this.jsonDef.name]: this._envelopeIdArray } }, 'omniactionbtn');
                            }
                        }
                    }
                }
        ...
        if (this.jsonDef) {
            this._actionUtilClass = new OmniscriptDocusignSignatureActionUtil(this.jsonDef);
        }
    }
    ...
    closeModal() {
        ...
        if (!this._iframeComp) this._iframeComp = this.template.querySelector('iframe');
