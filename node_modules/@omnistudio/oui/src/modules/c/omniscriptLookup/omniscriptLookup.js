...
import tmpl from './omniscriptLookup.html';
...
export default class OmniscriptLookup extends OmniscriptAtomicElement {
            ...
            this._actionUtilClass
                ...
                .then(resp => {
                    ...
                    if (remoteResp && remoteResp.options) {
                        if (!Array.isArray(remoteResp.options) && remoteResp.options.constructor === Object) {
    ...
    setSelected(selectedIndex) {
        if (this.options[selectedIndex] && this.options[selectedIndex].value !== '--') {
        ...
        // wait for value to be set into c-input then applyCallResp to trigger validation
     ...
     */
    showOptions() {
        this.template.querySelector(`.${this._theme}-dropdown-trigger_click`).classList.add(`${this._theme}-is-open`);
        this.show = true;
        this.isPageLoading = false;
    ...
    hideOptions() {
        ...
        this.template.querySelector(`.${this._theme}-dropdown-trigger_click`).classList.remove(`${this._theme}-is-open`);
        this.show = false;
    ...
    ariaFocus(newIndex) {
        ...
        if (options[newIndex]) {
            ...
            this._inputRef.setAttribute('aria-activedescendant', options[newIndex].id);
        }
    }
    ...
    generateUniqueAttributes() {
        if (this.options.length > 0 && !this.options[0].hasOwnProperty('key')) {
            for (let i = 0; i < this.options.length; i++) {
                this.options[i].key = i;
                this.options[i].id = `option-${i}`;
            }
        }
    }
    ...
    ndsUpdateLabel() {
        if (this._theme === 'nds') {
            if (this.lookupValue !== '' && this.lookupValue !== null) {
                //this.ndsLookupInput.classList.add('nds-not-empty', 'nds-is-dirty');
                this.inputClass += 'nds-not-empty nds-is-dirty';
            } else {
                //this.ndsLookupInput.classList.remove('nds-not-empty', 'nds-is-dirty');
                this.inputClass = this.inputClass.replace('nds-not-empty', '').replace('nds-is-dirty', '');
            }
        }
    }
    ...
    prefill(lookupValue) {
        ...
        if (this.options.length === 0 && this.jsonDef.options && this.jsonDef.options.length !== 0) {
            this.options = this.jsonDef.options;
        }
        ...
        if (lookupValue === undefined || lookupValue === null) {
            ...
            this.options.push({ name: lookupValue, value: lookupValue });
            this.saveOptions(this.options);
            this.lookupDisplay = lookupValue;
    ...
    @api setCustomValidation(message) {
        ...
        this.hasError = true;
        this.reportValidity();
        if (!this._onFocusOutErrorElementObj) {
            this._onFocusOutErrorElementObj = this.onFocusOutErrorElement.bind(this);
            this.template.addEventListener('focusout', this._onFocusOutErrorElementObj);
        }
    }
    ...
    onFocusOutErrorElement() {
        // removes custom validaton on focusout
    ...
    validateData(data) {
        // string or object
        return {
            valid: typeof data === 'string' || typeof data === 'object' || data === null,
            dataToApply: data,
        };
    }
    ...
    initCompVariables() {
        ...
        if (!this._isDesignMode && this.jsonDef.propSetMap.dataSource && this.jsonDef.propSetMap.dataSource.type === 'PicklistFilteredbyRecordType') {
            if (this.jsonDef.propSetMap.dataSource.picklistObjectAndField) {
                this.objectApiName = this.jsonDef.propSetMap.dataSource.picklistObjectAndField.split('.')[0];
            }
        }
        this.errorMessage = this._messageWhenValueMissing;
        this.errorClass =
            this._theme + '-form-element ' + this._theme + '-container_fluid' + (this.layout === 'newport' ? ' nds-form-container' : '');
        this.inputClass = 'vlocity-input ' + this._theme + '-input ' + (this.layout === 'newport' ? this._theme + '-input_mask ' : '');
    ...
    applyRepeatableStyles() {
        ...
        if (this.jsonDef && this._propSetMap && this._propSetMap.repeat) {
            if (this._theme === 'nds') {
                ...
                this._styleClasses.repeatAdd = 'nds-button_reset nds-button nds-button_icon nds-m-right_x-small';
                this._styleClasses.repeatIcons = `${prefix}-button__icon ${prefix}-button__icon_small `;
                this._styleClasses.repeatContainer = 'nds-button-group nds-m-right_x-small nds-tooltip__container omni-repeat-button-group';
                this._styleClasses.repeatRemove = 'nds-button_reset nds-button nds-button_icon ';
            } else {
