...
export function handleErrorReplace(errorMessage, elementProp, scriptHeaderDef) {
    ...
    const headerError = scriptHeaderDef && scriptHeaderDef.propSetMap ? scriptHeaderDef.propSetMap.errorMessage : null;
...
export function checkRemoteCallError(resp) {
    ...
    if (resp) {
        ...
        else if (!resp.error && !resp.records && resp.messages && Array.isArray(resp.messages) && resp.messages.length > 0) {
            resp.error = [];
...
export function findObjIdValues(obj, id) {
    ...
    for (objId in obj) {
        ...
        if (obj && obj.hasOwnProperty(objId) && typeof obj[objId] === 'object' && !Array.isArray(obj[objId])) {
 ...
 * @returns {object}
 * */
export function sendErrorModal(errorMessage, bBtn, element, scriptHeaderJson, comp) {
    ...
    if (
        ...
        previousLabel = undefined;
        dismissLabel = comp.scriptHeaderDef.allCustomLabels ? comp.scriptHeaderDef.allCustomLabels.OmniDesOk : 'OmniDesOk';
        nextLabel = undefined;
    } else {
                    ...
                    showToast(
                        {
                            ...
                            message: handleErrorReplace(errorMessage, comp._propSetMap, scriptHeaderJson),
                            variant: 'error',
                        },
                        this,
                    );
