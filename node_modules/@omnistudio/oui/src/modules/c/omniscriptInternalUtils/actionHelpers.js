...
import { isCommunityURL } from 'c/omniscriptRestApi';
/**
...
export const sendRequest = (obj, comp) => {
    return new Promise((res, rej) => {
        ...
        const url = obj.params ? `${obj.url}${obj.url.includes('?') ? '&' : '?'}${serializeObj(obj.params)}` : obj.url;
        ...
        xhr.ontimeout = () => {
            rej(comp.scriptHeaderDef.allCustomLabels ? comp.scriptHeaderDef.allCustomLabels.OmniRemoteCallTimeout : 'OmniRemoteCallTimeout');
        };
...
export const evalSpinnerCond = element => {
    ...
    if (!invokeMode || (invokeMode && noSpinnerConditions.indexOf(invokeMode) === -1)) {
        return true;
    }
...
export const setSpinnerActionMessage = element => {
    if (element.propSetMap.enableActionMessage && element.propSetMap.inProgressMessage) {
 ...
 * @returns {void}
 */
export function sendHttpDataToDebugConsole(comp, params, response, label, element, eventName) {
...
export function getUrl(page) {
    ...
    return isCommunityURL().then(isCommunity => {
        const pathName = isCommunity
            ...
            : document.location.pathname.substring(0, document.location.pathname.indexOf('/apex')) + '/apex/';
