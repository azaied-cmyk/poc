 ...
 * Build: v234.0.0
 */
import { mergeWith, isEqual, xorWith } from 'c/lodash';
...
function getStepPropSetLabel(step) {
    if (step && step.propSetMap && step.propSetMap.label) return step.propSetMap.label;
 ...
 */
export function scriptDataJsonInit(prefill, scriptJson, systemInfo, recordId, urlParams) {
    ...
    if (uiElements) {
        ...
        for (let i = 0; i < keyList.length; i++) {
            mergeIntoDataJson[keyList[i]] = scriptJson.propSetMap.seedDataJSON[keyList[i]];
...
export function evalCondition(control, type, comp, noCache) {
    var ctrlShowExprData = type === 'show' ? comp._showExprData : comp._validateExprData;
    ...
    if (ctrlShowExpression !== '()' && ctrlShowExpression !== '') {
        try {
            ...
            result = Function('data, mergeField', 'return ' + ctrlShowExpression)(data, mergeField);
        } catch (e) {
            window.console.error(`Invalid expression: function(data, mergeField) {\n\treturn ${ctrlShowExpression}(data, mergeField);\n}`);
        }
    }
...
function computed(group) {
    ...
    for (let i = 0; i < group.rules.length; i++) {
        ...
        if (group.rules[i].group) {
            ...
            str += "data['" + group.rules[i].field + "'] " + conditionMap[group.rules[i].condition] + ' ' + val;
...
export function handleMergeField(inputStr, jData, uiElements, JSONPath, bFormat, bNotEscape) {
    if (inputStr == null) {
        return '';
    }
    ...
    if (fields) {
        for (i = 0; i < fields.length; i++) {
            ...
            elemValue = getElementValue(elem, jData, uiElements, isRepeatNotation(elem) ? JSONPath : null, bFormat);
            {
                if (elemValue == null) {
                    ...
                    } else if (elemValue.constructor === Object || Array.isArray(elemValue)) {
                        resultName = resultName.split('"' + fields[i] + '"').join(JSON.stringify(elemValue));
                        resultName = resultName.split(fields[i]).join(JSON.stringify(elemValue));
                    } else {
...
function comparePath(pathArray1, pathArray2) {
    ...
    if (pathArray1 && pathArray2) {
        ...
        if (index != null && index > 0) {
            index--;
            returnArray = [...pathArray2.slice(0, index + 1), ...pathArray1.slice(index + 1, pathArray1.length)];
        }

        if (returnArray[returnArray.length - 1].length > 1 && returnArray[returnArray.length - 1][1] === 'n') {
...
export function processCloneNode(node, totalSize, maxCount, apiClone, rMap) {
    ...
    if (node.children && node.children.length > 0) {
        for (let i = 0; i < node.children.length; i++) {
            if (apiClone) {
                node.children[i].eleArray.splice(1, node.children[i].eleArray.length - 1);
            }
...
export function processChildNode(node, resetbInit, bNullifyResp, resetJSONPath, oldJSONPath, newJSONPath, newIndex, count, apiClone, rMap) {
    if (count != null) {
        node.ct = count;
    }
    ...
    if (resetJSONPath) {
        ...
        if (node.propSetMap.taAction) {
            node.propSetMap.taAction.JSONPath = node.propSetMap.taAction.JSONPath.replace(oldJSONPath, newJSONPath);
        }
    }
 ...
 * @returns {Object}
 */
export function getSendResponseJSON(response, jsonPath, jsonNode, eleJSONPath, uiElements, sendOnlyExtraPayload) {
    ...
    if (jsonPath) {
        ...
        if (propArray && propArray.length > 0) {
            resp = getElementValue(jsonPath, response, uiElements, isRepeatNotation(jsonPath) ? eleJSONPath : null, false) || {};
            rNode = propArray[propArray.length - 1][0];
        }
    } else {
        resp = response instanceof Array ? [...response] : Object.assign({}, response);
    }
 ...
 * @param value - driving value
 * @param comp - component
 */
export function autoAdvance(options, value, comp) {
    const itemResult = options.find(item => item.name === value);
    if (itemResult && itemResult.autoAdv != null) {
        comp.dispatchOmniEventUtil(comp, { moveToStep: itemResult.autoAdv }, 'omniautoadvance');
    }
}
...
export function isInBuilder() {
    return location.pathname.indexOf('flexipageEditor') > -1 || location.host.indexOf('livepreview') > -1;
}
