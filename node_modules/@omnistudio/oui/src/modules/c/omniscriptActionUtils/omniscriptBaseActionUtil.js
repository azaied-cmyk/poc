...
import { handleMergeField, getSendResponseJSON, isRepeatNotation, handleErrorReplace, handleExtraPayload } from 'c/omniscriptInternalUtils';
...
export class OmniscriptBaseActionUtil extends OmniscriptActionCommonUtil {
     ...
     * @returns {Object}
     */
    preProcessCommonReq(comp, element, params, input, vlcParams, bypassRemoteOptionsMf = false) {
        /**
        ...
        let options = element.propSetMap.remoteOptions ? Object.assign({}, element.propSetMap.remoteOptions) : params.remoteOptions || {};

        if (
            element.propSetMap.remoteClass &&
            element.propSetMap.remoteClass.indexOf('DefaultOmniScriptTypeAheadLookup') >= 0 &&
            comp &&
            comp.jsonDef &&
        ...
        let sClassName = (params && params.sClassName) || element.propSetMap.remoteClass;
        let sMethodName = (params && params.sMethodName) || element.propSetMap.remoteMethod;
        input = input || {};
        ...
        if (element && element.propSetMap.extraPayload && Object.getOwnPropertyNames(element.propSetMap.extraPayload).length > 0) {
            input = handleExtraPayload(element.propSetMap.extraPayload, input, comp, this.getCompJsonData(comp));
        }

        options.useQueueableApexRemoting = options.useQueueableApexRemoting || element.propSetMap.useQueueableApexRemoting === true;
    ...
    evaluateResp(element, resp, bSendToDebugConsole, comp) {
        ...
        const errorMsg = error ? handleErrorReplace(resp, comp._propSetMap, comp.scriptHeaderDef) : null;
    ...
    postProcess(resp, element, comp, failure) {
        if (failure) {
            const errorMessage = handleErrorReplace(resp, comp._propSetMap, comp.scriptHeaderDef);
        ...
        evaledResp.result = this.postProcessHelper(evaledResp.result, element, comp);
    ...
    postProcessHelper(resp, element, comp) {
        ...
        return this.getSendResponseJSONUtil(resp, element.propSetMap.responseJSONPath, element.propSetMap.responseJSONNode);
    }
