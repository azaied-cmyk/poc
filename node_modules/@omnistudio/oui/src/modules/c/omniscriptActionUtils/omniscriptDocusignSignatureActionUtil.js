...
import { getUrl } from 'c/omniscriptInternalUtils';
...
export class OmniscriptDocusignSignatureActionUtil extends OmniscriptDocusignEnvelopeActionUtil {
    ...
    getDataFromDocusignTemplate(control, comp) {
        ...
        if (control.propSetMap.docuSignTemplate && control.propSetMap.docuSignTemplate !== '') {
            ...
            for (let i = 0; i < control.propSetMap.docuSignTemplatesGroupSig.length; i++) {
                if (
    ...
    sendDocusignMail(comp, control, input) {
        ...
        return getUrl('OmniScriptDocuSignReturnPage').then(url => {
            ...
            if (comp.jsonDef.propSetMap.docuSignReturnUrl !== undefined && comp.jsonDef.propSetMap.docuSignReturnUrl !== '') {
    ...
    postProcessHelper(resp, element, comp) {
        ...
        if (resp.error !== 'OK') {
            ...
            comp.template.querySelector('iframe').setAttribute('src', resp[eleName].url);
            comp._envelopeId = resp[eleName].envelopeId;
            if (comp._envelopeId) {
                comp._envelopeIdArray.unshift({ status: 'In Process', envelopeId: comp._envelopeId });
            }
        }
