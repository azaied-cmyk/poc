 ...
 * Build: v234.0.0
 */
import { invokeApexMethod, sendOmniPubsubEvent, handleMessaging } from 'c/omniscriptUtils';
import { getNamespaceDotNotation, RUN_MODES } from 'c/omniscriptInternalUtils';
import { delay } from 'c/asyncUtils';
import { GenericInvoke2 } from 'c/omniscriptRestApi';
import { GenericInvoke2NoCont } from 'c/omniscriptRestApi';
...
export class OmniscriptActionCommonUtil {
    ...
    invokeAction(data, comp, payload) {
        ...
        if (data.error || data instanceof Promise) {
            return this.handlePromiseDuringInvoke(data.error ? Promise.resolve(data) : data, comp, payload);
        }
            ...
            this.callActionService(callConfig, this)
                .then(resp => {
                    ...
                    if (this._executedAsQueueable) {
                        ...
                        // node
                        resolve(this.executeAction(params, respObj.responseId, comp, payload));
                    } else {
                        if (
                            comp &&
                        ...
                        const processedResp = this.postProcess(respObj, this._element, comp);

                        // chainable support
                        if (processedResp && processedResp.result && typeof processedResp.result.then === 'function') {
                            resolve(
                                processedResp.result.then(vlcParams => {
                                    ...
                                    return this.executeAction(chainableParams, null, comp, payload, vlcParams);
                                }),
                            );
                        }
    ...
    getConfigForActionService(data) {
        ...
        return {
            ...
            apexMethod: data.options && data.options.useContinuation === true ? GenericInvoke2 : GenericInvoke2NoCont,
        };
    }
    ...
    handleActionMessagingEvents(comp, resp, params) {
        if (this._executedAsChainable === false && this._executedAsQueueable === false) {
            const eventData = {
            ...
            if (comp && comp._timeTrackingEnabled && comp.jsonDef.level === 0) {
                pubsub.fire(`${comp.scriptHeaderDef.uuid}omnitimetrackingdata`, 'timeTrackingData', {
                    component: comp,
            ...
            if ((comp && comp.jsonDef) || this._element) {
                ...
                delay(100).then(() => this.handleMessagingUtil(comp, 'omniscript_action', eventData, this._element, 'UI Action'));
            }
