...
import OmniscriptSetValues from 'c/omniscriptSetValues';
import { delay } from 'c/asyncUtils';

describe('c-omniscript-set-values', () => {
    const mockGetRecord = require('./mock.json');
    let element = null;
    beforeEach(() => {
        element = createElement('c-omniscript-set-values', {
            is: OmniscriptSetValues,
        });
    ...
    // eslint-disable-next-line @lwc/lwc/no-async-await
    it('Should receive omniactionbtn after clicking the button', async () => {
        expect.assertions(1);
        document.body.appendChild(element);
        let elementValueMap = element.jsonDef.propSetMap.elementValueMap;

        return new Promise(resolve => {
            const input = element.shadowRoot.querySelector('c-button');

            element.addEventListener('omniactionbtn', event => {
                expect(event.detail.apiResponse).toStrictEqual(elementValueMap);
                resolve();
            });
