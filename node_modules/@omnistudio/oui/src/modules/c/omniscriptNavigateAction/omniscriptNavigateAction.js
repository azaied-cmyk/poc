...
import { getNameSpacePrefix, isRepeatNotation } from 'c/omniscriptInternalUtils';
import tmpl from './omniscriptNavigateAction.html';
...
export default class OmniscriptNavigateAction extends OmniscriptBaseAction {
    ...
    @track targetType;
    getTargetType() {
        if (this._propSetMap.targetType === 'Vlocity OmniScript') return pageReferenceTypes.COMPONENT;
        ...
        return this._propSetMap.targetType
            ? this.handleMergeFieldUtil(
                  ...
                  isRepeatNotation(this._propSetMap.targetType) ? this.jsonDef.JSONPath : null,
              )
            : undefined;
    }
    ...
    getTargetId() {
        return this._propSetMap.targetId
            ? this.handleMergeFieldUtil(
                  ...
                  this._jsonData,
                  this.scriptHeaderDef.labelMap,
                  isRepeatNotation(this._propSetMap.targetId) ? this.jsonDef.JSONPath : null,
              )
            : undefined;
    }
            ...
            case this.targetType === pageReferenceTypes.WEB_PAGE:
                return this._propSetMap.targetUrl
                    ? this.handleMergeFieldUtil(
                          ...
                          this._jsonData,
                          this.scriptHeaderDef.labelMap,
                          isRepeatNotation(this._propSetMap.targetUrl) ? this.jsonDef.JSONPath : null,
                      )
                    : undefined;
            ...
            default:
                return this._propSetMap.targetName
                    ? this.handleMergeFieldUtil(
                          ...
                          this._jsonData,
                          this.scriptHeaderDef.labelMap,
                          isRepeatNotation(this._propSetMap.targetName) ? this.jsonDef.JSONPath : null,
                      )
                    : undefined;
        }
    }
        ...
        if (this._propSetMap.targetType === 'Vlocity OmniScript') {
            targetParams = this._propSetMap.targetLWCParams
                ? this.handleMergeFieldUtil(
                      ...
                      this._jsonData,
                      this.scriptHeaderDef.labelMap,
                      isRepeatNotation(this._propSetMap.targetLWCParams) ? this.jsonDef.JSONPath : null,
                  )
                : '';
            ...
            targetParams = this._propSetMap.targetParams
                ? this.handleMergeFieldUtil(
                      ...
                      this._jsonData,
                      this.scriptHeaderDef.labelMap,
                      isRepeatNotation(this._propSetMap.targetParams) ? this.jsonDef.JSONPath : null,
                  )
                : '';
        }
    ...
    constructor() {
        super();
        this.template.addEventListener('navigate', this.handleNavigateTrackingEvents.bind(this));
    }
    ...
    handleNavigateTrackingEvents(event) {
        ...
        if (!this._isDesignMode && event.detail) {
            ...
            if (this.jsonDef.level === 0 && this._timeTrackingEnabled) {
                pubsub.fire(`${this.scriptHeaderDef.uuid}omnitimetrackingdata`, 'timeTrackingData', {
                    component: this,
            ...
            if (this._navigateMessagingEnabled == null) {
                this._navigateMessagingEnabled = this.evaluateMessagingUtil(this.jsonDef.propSetMap, this.scriptHeaderDef.omniAnalyticsEnabled);
            }

            // Fire messaging for UI Action for Navigate Action
            if (this._navigateMessagingEnabled) {
                this.handleMessagingUtil(this, 'omniscript_action', eventData, this.jsonDef, 'UI Action');

                if (this.jsonData.vlocEvents) {
                    const eventArray = this.jsonData.vlocEvents.split(',');
                    if (eventArray && Array.isArray(eventArray)) {
                        for (let i = 0; i < eventArray.length; i++) {
                            this.handleMessagingUtil(this, eventArray[i], eventData, this.jsonDef, 'UI Action');
                        }
                    }
                }
            }
    ...
    stateRefresh() {
        ...
        if (this._isLink) Promise.resolve().then(() => this._navigateAction.generateUrl());
    }
    ...
    showErrorModal() {
        ...
        this.dispatchOmniEventUtil(this, { triggerValidation: true }, 'omniactionbtn');

        // triggers error for validation failure
        this.sendErrorModalUtil(this.allCustomLabelsUtil.OmniStepValidationError, this._isBtn, this.jsonDef, this.scriptHeaderDef, this);
    }
