 ...
 * Build: v234.0.0
 */
import OmniscriptTypeahead from 'c/omniscriptTypeahead';
import { getSendResponseJSON, setHttpDebugData } from 'c/omniscriptInternalUtils';
import { GoogleApiService, GooglePlacesService, calculateZoom, getCoordinates } from 'c/googlePlacesService';
import { isOffPlatform } from 'c/omniscriptRestApi';
import { track } from 'lwc';
import tmpl from './omniscriptPlacesTypeahead.html';
import { Place, COMPONENT_MAPPING, GOOGLE_ATTRIBUTION, PLACE_DETAIL_FIELDS } from './constants';
...
export default class OmniscriptPlacesTypeahead extends OmniscriptTypeahead {
    ...
    getOptions() {
        return this.getCoordinates()
            ...
            .then(result => {
                this.sendDataToDebugConsole('placeAutoComplete', { input: this.elementValue }, result, this._propSetMap.label);
            })
            .catch(reason => {
    ...
    setOptions(data) {
        if (data) {
            this.options = data.map(item => (item = { name: item.description, item: item }));
        } else {
            this.options = [];
        }
        return data;
    ...
    handleSelect(evt) {
        ...
        if (evt.detail.item && evt.detail.item.place_id) {
            return this._placesService
                .placeDetails(evt.detail.item.place_id, PLACE_DETAIL_FIELDS, this._sessionToken)
                .then(result => ({ ...result.result, place_id: evt.detail.item.place_id }))
                .then(result => {
                    this.sendDataToDebugConsole(
                        'placeDetails',
                        {
    ...
    applySelection(placeDetailsResult) {
        ...
        if (!this._propSetMap.hideMap && this._mapComp) {
            this.zoomLevel = calculateZoom(this._mapComp, placeDetailsResult.geometry.viewport);
            this.showMap();
        }
            ...
            if (mapping.hasOwnProperty(key)) {
                placeDetailsResult.address_components.forEach(component => {
                    if (component.types.includes(key) && mapping[key]) {
                        mapped[mapping[key]] = this.lodashUtil.get(component, COMPONENT_MAPPING[key], component.long_name);
                    }

                    if (mapping.street) {
                        if (component.types.includes('street_number')) street[0] = component.long_name || '';
                        if (component.types.includes('route')) street[1] = component.long_name || '';
                    }
                });

                if (mapping[key] && !mapped[mapping[key]]) {
                    mapped[mapping[key]] = getSendResponseJSON(placeDetailsResult, key, 'VlocityNoRootNode');
                }
            }
        }

        if (mapping.street) {
            mapped[mapping.street] = street.filter(item => item !== undefined).join(' ');
        }
    ...
    initCompVariables() {
        ...
        this.coordinatesPromise = getCoordinates().catch(reason => window.console.log('User location unavailable:', reason.message));
        if (this._propSetMap.hideMap && isOffPlatform() === false) this._renderMap = false; //For offplatform the map dom element must render.
        this.hideMap();
    }
    ...
    initialRenderCallback() {
        ...
        if (!this._propSetMap.hideMap && this.jsonDef.vlcPlace && this._mapComp) {
            ...
            this.zoomLevel = calculateZoom(this._mapComp, this.jsonDef.vlcPlace.geometry.viewport);
            this.showMap();
        }
    }
