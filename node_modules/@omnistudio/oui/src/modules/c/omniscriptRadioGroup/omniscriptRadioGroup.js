...
import tmpl from './omniscriptRadioGroup.html';
import tmpl_nds from './omniscriptRadioGroup_nds.html';
import { preprocessElementInput } from 'c/omniscriptInternalUtils';
...
export default class OmniscriptRadioGroup extends OmniscriptAtomicElement {
    ...
    handleChange(evt) {
        ...
        if (rowIndex) {
            ...
            if (!this._selectedValues[rowKey] || this._selectedValues[rowKey] !== evt.target.value) {
    ...
    selectRadio(labelIndex, optionIndex) {
        ...
        if (oldValue === null || oldValue !== newValue || !this._radioGroup[labelIndex].options[optionIndex].checked) {
    ...
    generateUniquePrefix() {
        ...
        if (pathArray && Array.isArray(pathArray)) {
            for (let i = 0; i < pathArray.length; i++) {
                if (pathArray[i] && Array.isArray(pathArray[i]) && pathArray[i].length === 2) {
    ...
    validateRadioGroup(showError) {
        ...
        if (this._propSetMap.required) {
            ...
            for (let i = 0; i < rlabels.length; i++) {
                if (!this._selectedValues || !(rlabels[i].name in this._selectedValues)) {
                    isValid = false;
    ...
    initCompVariables() {
        ...
        // error message
        this.errorMessage = this.allCustomLabelsUtil.OmniRequired;

        this._needMoreValidation = false;

        // sets classes for width of labels and options
        let lwidth = this._propSetMap.radioLabelsWidth;
        this._optionClass += ' ' + this._theme + '-grid ' + this._theme + '-size_' + (12 - lwidth) + '-of-12';
        this._labelClass += ' ' + this._theme + '-p-right_small ' + this._theme + '-size_' + lwidth + '-of-12';
    }
    ...
    connectedCallback() {
        ...
        if (!this._radioGroup) {
            this._radioGroup = JSON.parse(JSON.stringify(this._propSetMap.radioLabels));
