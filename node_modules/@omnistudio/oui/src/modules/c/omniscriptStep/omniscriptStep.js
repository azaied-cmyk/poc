...
import tmpl from './omniscriptStep.html';
...
export default class OmniscriptStep extends OmniscriptGroupElement {
     ...
     * @scope private
     */
    @track articleBodyResults = { articleBody: '', articleTitle: '', articleLink: '' };
    ...
    renderedCallback() {
        ...
        if (this._origActive !== undefined && newVal !== undefined && this._origActive !== newVal && newVal === false) {
        ...
        if (!newVal) {
            ...
            pubsub.unregister(`${this.scriptHeaderDef.omniscriptKey}omnikbarticlebodyinit`, this._kbArticleBodyInitializedObj);
            pubsub.unregister(`${this.scriptHeaderDef.omniscriptKey}omnikbarticlebodyresult`, this._kbArticleBodyResultObj);
        } else {
            super.renderedCallback();
        }

        if (this.articleBodyResults.setFocusToArticle) {
            const articleLink = this.template.querySelector('.vlc-slds-knowledge--header > a');
            if (articleLink) {
                articleLink.focus();
            }
    ...
    initialRenderCallback() {
        if (this.jsonDef.bAccordionActive && this.jsonDef.type === 'Step' && this.scriptHeaderDef.kbIndex >= 0) {
            this.articleBodyResults = { articleBody: '', articleTitle: '', articleLink: '' };
            if (this._propSetMap.showPersistentComponent[this.scriptHeaderDef.kbIndex]) {
                ...
                pubsub.register(`${this.scriptHeaderDef.omniscriptKey}omnikbinitilized`, this._kbInitializedObj);
                pubsub.register(`${this.scriptHeaderDef.omniscriptKey}omnikbarticlebodyinit`, this._kbArticleBodyInitializedObj);
                pubsub.register(`${this.scriptHeaderDef.omniscriptKey}omnikbarticlebodyresult`, this._kbArticleBodyResultObj);
            }
            this._toggleArticleLabel = this.scriptHeaderDef.allCustomLabels.OmniStepHideDetails;
            this._savedKnowledgeOptions = this.knowledgeOptions();
            pubsub.fire(`${this.scriptHeaderDef.omniscriptKey}omnikboptionschange`, 'data', this._savedKnowledgeOptions);
        }

        if (this.seedJson) {
            const apiJson = JSON.parse(JSON.stringify(this.seedJson));
            if (Object.getOwnPropertyNames(apiJson).length > 0) {
                this.dispatchOmniEventUtil(this, { apiResponse: apiJson, overwrite: true }, 'omniactionbtn');
            }
        }
        ...
        if (!this._isDesignMode && this.evaluateMessagingUtil(this.jsonDef.propSetMap, this.scriptHeaderDef.omniAnalyticsEnabled)) {
            ...
            Promise.resolve()
                ...
                .finally(() => {
                    ...
                    this.handleMessagingUtil(this, 'omniscript_step', messagingData, this.jsonDef, 'OS Step Load');
                });
        }
    }
    ...
    handleKnowledgeInitialized() {
        this._savedKnowledgeOptions = this.knowledgeOptions();
        pubsub.fire(`${this.scriptHeaderDef.omniscriptKey}omnikboptionschange`, 'data', this._savedKnowledgeOptions);
        pubsub.unregister(`${this.scriptHeaderDef.omniscriptKey}omnikbinitilized`, this._kbInitializedObj);
    ...
    stateRefresh() {
        if (
            ...
            if (!this.lodashUtil.isEqual(options, this._savedKnowledgeOptions))
                pubsub.fire(`${this.scriptHeaderDef.omniscriptKey}omnikboptionschange`, 'data', options);
     ...
     */
    knowledgeOptions() {
        const isKbEnabledOnStep = this._propSetMap.showPersistentComponent[this.scriptHeaderDef.kbIndex];
        ...
        if (_step && _step.type === 'Step') {
            ...
            if (this.scriptHeaderDef && this.scriptHeaderDef.propSetMap) {
                kbObject = this.scriptHeaderDef.propSetMap.persistentComponent.find(({ id }) => id === 'vlcKnowledge');
            }
            this.knowledgeLabel = this.scriptHeaderDef && this.scriptHeaderDef.propSetMap && kbObject ? kbObject.label : '';
            ...
            //mergefield
            if (optionKeyword) {
                keyword = this.handleMergeFieldUtil(optionKeyword, this._jsonData, this.scriptHeaderDef.labelMap, null);
            }

            const sClassName = getNamespaceDotNotation() + 'DefaultKnowledgeOmniScriptIntegration';
            ...
            kbOptionsParam = {
                ...
                knowledgeArticleTypeQueryFieldsMap: this.scriptHeaderDef.propSetMap.knowledgeArticleTypeQueryFieldsMap,
    ...
    knowledgeArticleOptions(article) {
        ...
        if (_step && _step.type === 'Step') {
            ...
            const searchFlds = this.scriptHeaderDef.propSetMap.knowledgeArticleTypeQueryFieldsMap;
            if (searchFlds) {
    ...
    handleArticleBodyInitialized(data) {
        pubsub.fire(`${this.scriptHeaderDef.omniscriptKey}omnikbarticlebodychange`, 'data', this.knowledgeArticleOptions(data));
    }
     ...
     */
    toggleArticle() {
        const { OmniStepHideDetails, OmniStepShowDetails } = this.scriptHeaderDef.allCustomLabels;
        this.visible = !this.visible;
        this._toggleArticleLabel = this.visible ? OmniStepHideDetails : OmniStepShowDetails;
    }

    getBoundsForDesigner() {
        const container = this.template.querySelector(
            this.layout === 'lightning' ? '.omniscript-step__body > .slds-grid' : '.omniscript-step__body-nds',
        );
        const outerWrapper = this.getBoundingClientRect();
        if (container) {
            const computedStyle = getComputedStyle(container);
            const boundingRect = container.getBoundingClientRect();
            const bounds = Object.assign({}, JSON.parse(JSON.stringify(outerWrapper)), {
            ...
            bounds.containerBounds.height = this.safeParseStyleToNumber(getComputedStyle(container.parentNode).height);
            return bounds;
        }
