...
import { AggregatesValidation, VALIDATION_EVENTS } from 'c/omniscriptValidation';
...
import { sendOmniPubsubEvent, addPathToAPIJson, handleMessaging } from 'c/omniscriptUtils';
import { evaluateMessaging } from 'c/omniscriptTrackingServiceUtils';
 ...
 */
export default class OmniscriptGroupElement extends AggregatesValidation(OmniscriptBaseElement) {
    /**
    ...
    constructor() {
        ...
        this.template.addEventListener('omnirefreshseeddata', this.handleOmniSeedDataRefresh.bind(this));
        this.template.addEventListener('omnisetshow', this.handleOmniSetShow.bind(this));
        this.template.addEventListener('omnirepeat', this.handleOmniRepeat.bind(this));
        this.template.addEventListener('omniupdatejsondef', this.handleOmniUpdateJsonDef.bind(this));
    }
    ...
    @api applyCallResp(data, bApi = false, bValidation = false) {
        ...
        if (data != null && (!this.lodashUtil.isEqual(this._elementValueObj, data) || bValidation || Object.keys(this.invalidElements).length > 0)) {
            // OWC-182
            ...
            const leftOverJson = this.applyCallRespUtil(data, this, bApi, bValidation, !this.runServerCheck());

            if (leftOverJson) {
                if (!bValidation && leftOverJson.remainder && Object.getOwnPropertyNames(leftOverJson.remainder).length > 0) {
                    try {
                        this.lodashUtil.mergeWith(this._elementValueObj, leftOverJson.remainder, this.mergeJSONLogicUtil);
                    } catch (error) {
                    ...
                    if (this.jsonDef.sOmniScriptId) {
                        ...
                        this._jsonData = this.jsonDef.response = Object.assign({}, this._elementValueObj);
                        this.headerRefresh();

                        // Handles messaging for data JSON changes
                        if (!this._isDesignMode && this.evaluateMessagingUtil(this.scriptHeaderDef.propSetMap)) {
                ...
                if (leftOverJson.cacheJson && Object.getOwnPropertyNames(leftOverJson.cacheJson).length > 0) {
                    // header
                    if (this.jsonDef.sOmniScriptId) {
                        this.lodashUtil.mergeWith(this.jsonDef.propSetMap.seedDataJSON, leftOverJson.cacheJson, this.mergeJSONLogicUtil);
                    } else {
                        this.dispatchOmniEventUtil(this, { mergeJson: leftOverJson.cacheJson }, 'omnirefreshseeddata');
                    }
                }
            }
    ...
    handleOmniAggregate(evt) {
        ...
        if (evt.detail && evt.detail.elementId && evt.detail.elementId !== this._elementId) {
            ...
            const evtData = evt.detail.data != null ? JSON.parse(JSON.stringify(evt.detail.data)) : evt.detail.data;
            ...
            if (index > 0) {
                if (evt.detail.operation === 'delete' && Array.isArray(aggNodeVal)) {
                    ...
                    newNodeVal = [...newNodeVal.slice(0, index), ...newNodeVal.slice(index + 1, newNodeVal.length)];
                } else {
                    if (aggNodeVal == null) {
                        newNodeVal = [];
                    ...
                    const insertInd = evt.detail.operation === 'add' && !bInserted ? index : index + 1;
                    // update index=evt.detai.index element
                    newNodeVal = [...newNodeVal.slice(0, index), evt.detail.show !== false ? evtData : null, ...newNodeVal.slice(insertInd)];
                }
                ...
                if (evt.detail.operation === 'delete') {
                    ...
                    if (evt.detail.repeat && aggNodeVal && Array.isArray(aggNodeVal)) {
                        ...
                        newNodeVal = [evt.detail.show !== false ? evtData : null, ...newNodeVal.slice(index + 1)];
                    }
                }
            }
            ...
            if (this.jsonDef.sOmniScriptId) {
                ...
                this.jsonDataStr = JSON.stringify(this._elementValueObj);
                this._jsonData = this.jsonDef.response = Object.assign({}, this._elementValueObj); // trigger rerender
                this.headerRefresh();

                // we also need to update response at different levels
                this.updateJsonDefUtil(this.jsonDef, evt.detail.aggregateNodesPath, 'response');
                ...
                if (evt.detail.operation === 'newClone') {
                    ...
                    this.updateJsonUtil(this.jsonDef, elePath, 'delete', [{ nodeName: 'newClone', newData: null }]);
                }
            ...
            let operationBubbleUp = evt.detail.operation === 'add' || evt.detail.operation === 'newClone' ? 'newClone' : undefined;
            this.dispatchOmniEventUtil(this, this.createAggregateNode(false, operationBubbleUp), 'omniaggregate');
        }
    }
    ...
    handleOmniSeedDataRefresh(evt) {
        if (this.jsonDef.sOmniScriptId && evt.detail) {
            ...
            if (evt.detail.path) {
                this.updateJsonUtil(this.jsonDef.propSetMap.seedDataJSON, '', 'delete', [{ nodeName: evt.detail.path }]);
            }
            // this is to merge in cache json for each apply
            if (evt.detail.mergeJson) {
                this.lodashUtil.mergeWith(this.jsonDef.propSetMap.seedDataJSON, evt.detail.mergeJson, this.mergeJSONLogicUtil);
            }
        }
    }
        ...
        if (evt.detail && evt.detail.elementId && evt.detail.elementId !== this._elementId) {
            ...
            if (!this.jsonDef.sOmniScriptId) {
                ...
                this.dispatchOmniEventUtil(
                    this,
                    { path: aggPath, elementId: this._elementId, value: evt.detail.value, node: evt.detail.node, operation: evt.detail.operation },
                    'omniupdatejsondef',
                );
            } else {
                if (evt.detail.operation === 'delete') {
                    this.updateJsonUtil(this.jsonDef, evt.detail.path, 'delete', [{ nodeName: evt.detail.node }]);
                } else {
                    updateJsonDef(this.jsonDef, [{ value: evt.detail.value, path: evt.detail.path }], evt.detail.node);
                }
            }
        }
    }
    ...
    handleOmniSetShow(evt) {
        if (evt.detail && evt.detail.elementId && evt.detail.elementId !== this._elementId) {
            ...
            if (!this.jsonDef.sOmniScriptId) {
                let aggPath = this._jsonPath + ':' + evt.detail.path;
                this.dispatchOmniEventUtil(this, { path: aggPath, value: evt.detail.value, elementId: this._elementId }, 'omnisetshow');
            }
        }
    }
    ...
    createAggregateNode(bFixProxy, operation) {
        ...
        if (bFixProxy && !this.jsonDef.sOmniScriptId) {
            detail.data = detail.data != null ? JSON.parse(JSON.stringify(detail.data)) : null;
        }
        return detail;
    }
        ...
        if (evt.detail && evt.detail.elementId && evt.detail.elementId !== this._elementId) {
            if (!this.jsonDef.sOmniScriptId) {
                ...
                let pathSplit = evt.detail.path.split('|'),
                    ...
                    eleNode = getJSONNode(this.jsonDef, preprocessElementInput(evt.detail.path)),
                    eleArrHeader = getJSONNode(this.jsonDef, preprocessElementInput(elePath)),
                    operation = evt.detail.operation;
                let bUpdate = false;
                let midIndex;
                let offSet;
                ...
                if (operation === 'update') {
                    ...
                    eleArrHeader.mct++;
                    processCloneNode(eleNode, eleArrNodes.length, eleArrHeader.mct, evt.detail['$Vlocity.seed'] != null, this.scriptHeaderDef.rMap);
                    if (evt.detail['$Vlocity.seed']) {
                        eleNode['$Vlocity.seed'] = evt.detail['$Vlocity.seed'];
                        delete eleNode.newClone;
                    }
                    eleArrNodes = [...eleArrNodes.slice(0, newIndex), eleNode, ...eleArrNodes.slice(newIndex)];
                    bUpdate = true;
                    midIndex = newIndex + 1;
                    offSet = 2;
                } else if (operation === 'delete') {
                    eleArrNodes = [...eleArrNodes.slice(0, newIndex - 1), ...eleArrNodes.slice(newIndex)];
                    bUpdate = true;
                    midIndex = newIndex - 1;
                    offSet = 0;
                }
        ...
        if (this._isBtn || this._isBtn === undefined) {
            if (evalSpinnerCond(element)) {
                if (value === true) {
                    ...
                    this.spinnerActionMessage = setSpinnerActionMessage(element);
                }
    ...
    validityHook(newShow) {
        if (this.jsonDef.level === 0 && this.jsonDef.name !== this.scriptHeaderDef.asName) return;
    ...
    markInputAsInvalid(evt) {
        ...
        if (this.jsonDef.bShow === true || (!this.jsonDef.propSetMap.show && !this.jsonDef.bShow)) {
...
function removeVlcPlace(elementString) {
    ...
    if (elementString.substring(start, start + skipCase.length) === skipCase) {
        ...
        for (let current = start + open.length; current < elementString.length; current++) {
    ...
    return elementString.slice(0, start) + elementString.slice(end, elementString.length);
}
