...
import { createGlobalActionList, displayErrorModal, runAction } from 'c/omniscriptEditBlockUtils';
import pubsub from 'c/pubsub';
import { handleMultiLangLabel } from 'c/omniscriptCustomLabels'; // will be removed and will use getCustomLabel, I think.
...
export default class OmniscriptEditBlock extends OmniscriptGroupElement {
    ...
    handleAdd(evt, seed) {
        ...
        if (
            ...
            (!this.jsonDef.childrenC || (this.jsonDef.childrenC && this.jsonDef.children.length !== this.jsonDef.childrenC.length))
        ) {
            if (evt !== 'api') {
                this.isEditing = true;
                this._isNew = true;
                ...
                buttons: [
                    {
                        ...
                        key: '0-Cancel',
                        handleClick: () => {
                            let focusOnCancel = this.template.querySelector(`[data-remove-button]`);
                            if (!focusOnCancel) {
                                focusOnCancel = this.template.querySelector('[data-action-button-menu');
                            }
    ...
    cancel() {
        if (this._isNew) {
            ...
            const modifiedOldData = this.createNullifiedJsonData(this._oldData, this.jsonDef.response ? this.jsonDef.response : {});
    ...
    updateSiblings(data) {
        ...
        if (this._isCards && data && data.add && data.addIndex === this.jsonDef.index) {
            this.handleAdd();
        } else if (data && data.checkbox && data.checkboxIndex !== this.jsonDef.index) {
    ...
    save() {
        if (this.reportValidity()) {
            ...
            const invokeAction = this._isNew ? this._propSetMap.newAction : this._propSetMap.editAction;
                ...
                runAction(this, invokeAction).then(resp => {
                    if (!resp.error) {
                        ...
                        Promise.resolve().then(() => {
                            const firstFocusableEl = this.template.querySelector('[tabindex="0"]');
                            if (firstFocusableEl) {
    ...
    toggleActionMenu(evt) {
        ...
        if (this._menuExpanded) {
            Promise.resolve().then(() => {
                const firstMenuItem = this.template.querySelector(`ul.${this._theme}-dropdown__list li:first-child > a`);
                if (firstMenuItem) {
                    firstMenuItem.focus();
                }
            });
    ...
    handleUpKey() {
        ...
        const firstItem = this.template.querySelector(`ul.${this._theme}-dropdown__list li:first-child`);
        const lastItem = this.template.querySelector(`ul.${this._theme}-dropdown__list li:last-child`);
        let preElement;
        if (parent === firstItem) {
            preElement = lastItem;
        } else {
    ...
    initCompVariables() {
        ...
        this._maxDisplayCount = this._propSetMap.maxDisplay ? this._propSetMap.maxDisplay : this._maxDisplayCount;
        ...
        if (!this._updateSiblingsPubsubObj) {
            ...
            pubsub.register(this.jsonDef.name + '_' + this.scriptHeaderDef.uuid, this._updateSiblingPubsubObj);
        }
    ...
    propToMultiLang(properties) {
        ...
        if (this._propSetMap.gActions) {
            this._propSetMap.gActions.forEach(gAction => {
                handleMultiLangLabel(gAction.type, gAction.propSetMap, this.allCustomLabelsUtil);
            });
        }
        if (this._propSetMap.actions) {
            this._propSetMap.actions.forEach(action => {
                handleMultiLangLabel(action.type, action.propSetMap, this.allCustomLabelsUtil);
            });
        }
        if (this._propSetMap.newAction) {
            handleMultiLangLabel(this._propSetMap.newAction.type, this._propSetMap.newAction.propSetMap, this.allCustomLabelsUtil);
        }
        if (this._propSetMap.editAction) {
            handleMultiLangLabel(this._propSetMap.editAction.type, this._propSetMap.editAction.propSetMap, this.allCustomLabelsUtil);
        }
        if (this._propSetMap.delAction) {
            handleMultiLangLabel(this._propSetMap.delAction.type, this._propSetMap.delAction.propSetMap, this.allCustomLabelsUtil);
        }
    }
    ...
    createDisplayColumns() {
        if (this.jsonDef && this.jsonDef.children && this.jsonDef.children.length > 0 && this.jsonData) {
            this._displayValues = [];
            let preDisplayVal = [];
            ...
            for (let i = 0; i < this.jsonDef.children.length; i++) {
                ...
                if (eleArray.propSetMap.disOnTplt && this._maxDisplayCount > preDisplayVal.length) {
            ...
            for (let i = 0; i < preDisplayVal.length; i++) {
                ...
                let value = this.jsonData.OmniScriptFmtData && this.jsonData.OmniScriptFmtData[eleArray.JSONPath];
                ...
                if (this._isFS || this._isTable) {
                    ...
                    if (this._isFS) {
                        ...
                        width = this._tableWidth[preDisplayVal.length] ? this._tableWidth[preDisplayVal.length] : 1;
                    }
                    cls += ` ${this._theme}-size_${width}-of-12`;

                    if (eleArray.type === 'Checkbox') {
                        cls += ` ${this._theme}-grid ${this._theme}-grid_align-center`;
                    } else if (eleArray.type === 'Currency' || eleArray.type === 'Number') {
    ...
    updateDisplayColumns() {
        if (this.jsonDef && this.jsonDef.children && this.jsonDef.children.length > 0 && this.jsonData) {
            if (this._displayValues.length < 1) {
                ...
                for (let i = 0; i < this.jsonDef.children.length; i++) {
                    let eleArray = this.jsonDef.children[i].eleArray[0];
                    if (eleArray.propSetMap.disOnTplt && dvIndex < this._displayValues.length) {
                        let value = this.jsonData.OmniScriptFmtData && this.jsonData.OmniScriptFmtData[eleArray.JSONPath];
                        value = value == null ? eleArray.response : value;
                        value = value == null ? '' : value;
    ...
    createTableLabelColumns() {
        if (this.jsonDef) {
            ...
            for (let i = 0; i < children.length; i++) {
                ...
                // Terence, we might need to check the case where controls under Edit Block becomes repeatable
                if (eleArray.propSetMap.disOnTplt && this._maxDisplayCount > preTableLabels.length) {
            ...
            for (let i = 0; i < preTableLabels.length; i++) {
                ...
                if (this._isFS || this._isTable) {
                    ...
                    if (this._isFS) {
                        ...
                        // automatically determine width of each column based on display count
                        width = this._tableWidth[preTableLabels.length] ? this._tableWidth[preTableLabels.length] : 1;
                    }
                    cls += ` ${this._theme}-size_${width}-of-12`;
                ...
                let tableLabel =
                    eleArray.propSetMap.label && this._multiLang ? this.allCustomLabelsUtil[eleArray.propSetMap.label] : eleArray.propSetMap.label;
                ...
                for (let i = 0; i < keys.length; i++) {
                    ...
                    if (newdata && newdata[propKey]) {
                        ...
                        // nullify non-object properties, restore existing data saved before editing
                            ...
                            olddata[propKey] = olddata[propKey] != null ? olddata[propKey] : null;
                        }
                    }
                }
            }
    ...
    get showAdd() {
        ...
        return (
            this._allowNew &&
            !this._isEditing &&
            ((this.jsonDef.childrenC && this.jsonDef.children.length !== this.jsonDef.childrenC.length) || this.jsonDef.index === this.jsonDef.ct - 1)
        );
    }
    ...
    get svgMapIcon() {
        let svgIcon = '';
        if (this.jsonDef) {
            if (this.jsonDef.response && this._propSetMap.valueSvgMap && this._propSetMap.elementName) {
                let svgMap = this._propSetMap.valueSvgMap;
                for (let i = 0; i < svgMap.length; i++) {
                    if (svgMap[i].value === this.jsonDef.response[this._propSetMap.elementName]) {
                        svgIcon = svgMap[i].svgSprite + ':' + svgMap[i].svgIcon;
                    }
                }
            }
            svgIcon = svgIcon !== '' ? svgIcon : this._propSetMap.svgSprite + ':' + this._propSetMap.svgIcon;
        }
        return svgIcon;
    }
    ...
    get visualClass() {
        ...
        return this._theme + '-size_1-of-1 ' + this._theme + (this._isEditing || !this._hasChildren ? '-hide' : '-show');
    }
     ...
     */
    get editClass() {
        return this._theme + '-size_1-of-1 ' + this._theme + (this._isEditing ? '-show' : '-hide');
    }

    applyCtrlWidth() {
        super.applyCtrlWidth();
        if (this.jsonDef && this._propSetMap) {
            this.classList.remove(this._theme + '-medium-size_' + this._propSetMap.controlWidth + '-of-12');
    ...
    disconnectedCallback() {
        if (this._updateSiblingPubsubObj) {
            pubsub.unregister(this.jsonDef.name + '_' + this.scriptHeaderDef.uuid, this._updateSiblingPubsubObj);
        }
        ...
        if (this.previousFirstFocusableElement) {
            this.previousFirstFocusableElement.removeEventListener('keydown', this._handleKeyDownOnFirstElementInModal);
        }
    }

    @api
    applyCallResp(data, bApi = false, bValidation = false, bSkipPromise, bRefreshUI) {
    ...
    @api
    isEmpty() {
        return this.jsonDef.index === 0 && this.jsonDef.children && this.jsonDef.children.length !== this.jsonDef.childrenC.length;
    }
    ...
    renderedCallback() {
        ...
        if (
            ...
            this._showCheckbox = this.jsonDef.response[checkboxName];
            if (this._propSetMap.selectMode === 'Single') {
                pubsub.fire(this.jsonDef.name + '_' + this.scriptHeaderDef.uuid, 'data', { checkbox: true, checkboxIndex: this.jsonDef.index });
            }
        }

        if (!this.lastFocusableElement) {
            if (!this._isInline) {
                this.lastFocusableElement = this.template.querySelector('[role="dialog"] button:last-child');
            } else if (this._isInline && this.isEditing) {
                this.lastFocusableElement = this.template.querySelector('button:last-child');
            }
            if (this.lastFocusableElement) {
        ...
        if (this.previousFirstFocusableElement !== this.firstFocusableElement && this.firstFocusableElement) {
            if (this.previousFirstFocusableElement) {
                this.previousFirstFocusableElement.removeEventListener('keydown', this._handleKeyDownOnFirstElementInModal);
            }
