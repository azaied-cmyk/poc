...
@include deprecate('4.0.0', 'Use slds-avatar__initials_inverse instead') {
  ...
  .slds-chat-avatar__intials {
    background-color: $chat-message-color-background-inbound;
...
.slds-chat-message__text_outbound {
  margin-left: auto;
  background-color: $chat-message-color-background-outbound;
}
 ...
 */
.slds-chat-message__text_outbound-agent {
  background-color: $chat-message-color-background-outbound-agent;
  margin-left: auto;
}
...
@include deprecate('4.0', 'Horizontal rules are no longer used on events') {
  ...
  .slds-chat-event__rule {
    border-top: $border-width-thin $color-border solid;
    height: rem(1px);
    flex-grow: 1;
  }
}
