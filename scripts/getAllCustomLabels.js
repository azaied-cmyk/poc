...
function findFileHandler(filepath) {
	...
	const modulepath = filepaths.join('/');
...
function getOuiModulePath() {
	...
	const data = fs.readFileSync('./lwc.config.json', "utf8");
	let path;
	try {
		const lwcConfig = JSON.parse(data);
		// find the npm path for oui
		path = lwcConfig.modules.filter(value => value.npm)
			  .reduce((acc, value) => {
...
const filesFound = {};
const labelKeyMapOriginalPath = './node_modules/' + ouiModulePath + '/src/modules/c/omniscriptRestApi/customLabels.js';
const pathToTranslations = 'src/modules/vlocitytranslations';
...
findFile('./src/modules', /MultiLanguage\/.*_def.js/, findFileHandler);
findFile('./src/modules', /MultiLanguage\/omnidef_chunk\d*.js/, findFileHandler);
